...
import io.airlift.compress.zstd.ZstdFrameDecompressor.DecompressState.State;
...
class ZstdFrameDecompressor
{
    ...
    public static class DecompressState
    {
        public static final DecompressState INITIAL_DECOMPRESS_STATE = new DecompressState();
        ...
        final State state;
        final boolean atStoppingPoint;
        final FrameHeader frameHeader;
        final int blockHeader;

        final int inputConsumed;
        final int outputBufferUsed;

        final int inputRequired;
        final int requestedOutputSize;

        private DecompressState()
        {
            this.state = State.READ_FRAME_MAGIC;
            this.atStoppingPoint = false;
        ...
        DecompressState(State state, FrameHeader frameHeader, int blockHeader, int inputConsumed, int outputBufferUsed, int inputRequired, int requestedOutputSize)
        {
            this.state = requireNonNull(state, "state is null");
            this.atStoppingPoint = state == State.READ_FRAME_MAGIC;
            this.frameHeader = frameHeader;
            this.blockHeader = blockHeader;

            checkArgument(inputConsumed >= 0, "inputConsumed is negative");
            this.inputConsumed = inputConsumed;
            checkArgument(outputBufferUsed >= 0, "outputBufferUsed is negative");
            this.outputBufferUsed = outputBufferUsed;
            checkArgument(inputRequired >= 0, "inputRequired is negative");
        ...
        {
            return new StringJoiner(", ", DecompressState.class.getSimpleName() + "[", "]")
                    .add("state=" + state)
                    .add("frameHeader=" + frameHeader)
                    .add("blockHeader=" + blockHeader)
                    .add("inputConsumed=" + inputConsumed)
                    .add("outputBufferUsed=" + outputBufferUsed)
                    .add("inputRequired=" + inputRequired)
                    .add("requestedOutputSize=" + requestedOutputSize)
                    .toString();
        }
    }

    // current window buffer
    private Object windowBase = new byte[0];
    ...
    public DecompressState partialDecompress(
    ...
    {
        if (decompressState.inputRequired > inputLimit - inputAddress) {
            throw new IllegalArgumentException(format(
                    "Expected %s input are required, but only %s input bytes were supplied",
                    decompressState.inputRequired,
                    inputLimit - inputAddress));
        }
        if (decompressState.getRequestedOutputSize() > 0 && outputOffset >= outputLimit) {
            throw new IllegalArgumentException("Output is ready but output buffer is full");
        }

        long input = inputAddress;
        int output = outputOffset;

        State state = decompressState.state;
        FrameHeader frameHeader = decompressState.frameHeader;
        while (true) {
            // Flush ready output
            {
                ...
                if (flushableOutputSize > 0) {
                    ...
                    if (flushableOutputSize > 0) {
                        return new DecompressState(state, frameHeader, -1, (int) (input - inputAddress), output - outputOffset, 0, flushableOutputSize);
                    }
                }
            }
            ...
            if (state == State.READ_FRAME_MAGIC) {
                if (inputLimit - input < 4) {
                    return new DecompressState(state, null, -1, (int) (input - inputAddress), output - outputOffset, 4, 0);
                }
                input += verifyMagic(inputBase, input, inputLimit);
            ...
            if (state == State.READ_FRAME_HEADER) {
                if (inputLimit - input < 1) {
                    return new DecompressState(state, null, -1, (int) (input - inputAddress), output - outputOffset, 1, 0);
                }
                int frameHeaderSize = determineFrameHeaderSize(inputBase, input, inputLimit);
                if (inputLimit - input < frameHeaderSize) {
                    return new DecompressState(state, null, -1, (int) (input - inputAddress), output - outputOffset, frameHeaderSize, 0);
                }
            ...
            if (state == State.READ_BLOCK_HEADER) {
                ...
                if (inputBufferSize < SIZE_OF_BLOCK_HEADER) {
                    return new DecompressState(state, frameHeader, -1, (int) (input - inputAddress), output - outputOffset, SIZE_OF_BLOCK_HEADER, 0);
                }
                ...
                blockHeader = decompressState.blockHeader;
            }
                ...
                switch (blockType) {
                    case RAW_BLOCK: {
                        if (inputLimit - input < blockSize) {
                            return new DecompressState(state, frameHeader, blockHeader, (int) (input - inputAddress), output - outputOffset, blockSize, 0);
                        }
                    ...
                    case RLE_BLOCK: {
                        if (inputLimit - input < 1) {
                            return new DecompressState(state, frameHeader, blockHeader, (int) (input - inputAddress), output - outputOffset, 1, 0);
                        }
                        verify(windowLimit - windowPosition >= blockSize, input, "window buffer is too small");
                    ...
                    case COMPRESSED_BLOCK: {
                        ...
                        decodedSize = decodeCompressedBlock(inputBase, input, blockSize, windowBase, windowPosition, windowLimit, frameHeader.windowSize, windowAddress);
                        input += blockSize;
                        break;
                    }
                    default:
            ...
            if (state == State.READ_BLOCK_CHECKSUM) {
                if (frameHeader.hasChecksum) {
                    if (inputLimit - input < SIZE_OF_INT) {
                        return new DecompressState(state, frameHeader, blockHeader, (int) (input - inputAddress), output - outputOffset, SIZE_OF_INT, 0);
                    }
                    ...
                    partialHash.update((byte[]) windowBase, toIntExact(windowAddress - ARRAY_BYTE_BASE_OFFSET), pendingOutputSize);
    ...
    private void resizeWindowBufferIfNecessary(FrameHeader frameHeader, int blockType, int blockSize)
    {
        ...
        if (windowLimit - windowPosition < MAX_BLOCK_SIZE) {
            ...
            if (windowLimit - windowPosition < maxBlockOutput) {
                ...
                windowBase = Arrays.copyOf((byte[]) windowBase, newWindowSize);
    ...
    static int determineFrameHeaderSize(final Object inputBase, final long inputAddress, final long inputLimit)
    {
